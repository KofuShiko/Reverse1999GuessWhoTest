...
function Buttons({categoryNames, excluded, onToggle, reset, hideToggle, hidden}) {
    return(
        <section className="sticky top-0 z-10 flex flex-wrap justify-between gap-3 p-2 py-4 bg-linear-to-b from-black to-transparent text-white/80 text-xs">
            <div>
                <span className='text-xl'>Reverse:1999</span>
                <h1 className="text-xs"> Guess Who</h1>
            </div>
            <div className='*:m-1 *:rounded-md *:p-1 *:px-3 *:font-bold *:hover:text-white/90 *:duration-300'>
            {categoryNames && categoryNames.map((category) => {
                ...
                <button onClick={hideToggle} className="text-xs">{hidden ? 'Show All' : 'Hide Grayed'}</button>
            </div>
